...
string generate(string code, generate_flags flags, vector<generate_error> &err ) {
	...
	if (flags.plus) {
		...
		if (!flags.minimize) {
			...
			if(flags.accept_elvm) result += "#include<stdio.h>\n#include<ctype.h>\nunsigned char x;int temp;int p=0;int readint(){char c;int r=0;while(isspace(c=getchar()));r=c-48;while(isdigit(c=getchar()))r=r*10+c-48;return r;}int main(){";
			else result += "#include<stdio.h>\nunsigned char x;int temp;int p=0;int main(){";
		}
	}
